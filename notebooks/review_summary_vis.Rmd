...
```{r data, cache = TRUE, message = FALSE, results = "hide"}
...
View the rating distribution over time for the 9 recalled products that have the
most reviews.

First find the recalled products with the most reviews

```{r most-reviewed-recalled-products}
...
Subset review data to only include these product. Then find the quarterly count
of each star rating (1-5) for each product.

```{r quarterly-rating-tallies}
...
Visualize quarterly rating distribution for the `r length(top_asins)` most reviewed recalled products.

```{r quarterly-rating-dist-recalled}
